...
class DataInput:
	def __init__(self, data, u_read_list, u_friend_list, uf_read_list, u_vector, i_read_list, i_friend_list, if_read_list, \
		i_vector, i_link_list, batch_size, trunc_len):
	...
	def __next__(self):
		...
		u_read_l, u_friend_l, uf_read_linput = [], [], []
		i_read, i_friend, if_read = [], [], []
		i_read_l, i_friend_l, if_read_linput, i_link = [], [], [], []
		u_vector = []
		i_vector = []
		u_att1, u_att2, u_vector_l = [], [], []
		i_att1, i_att2, i_vector_l = [], [], []
		...
		for t in ts:
			...
			#u_att1.append(t[0])
			#u_att2.append(t[1])
			#u_vector_l.append(t[2])

			#i_att1.append(t[0])
			#i_att2.append(t[1])
			#i_vector_l.append(t[2])
			...
			#i_vector_u = self.i_vector(t[1])
			#if len(i_vector_u) <= self.trunc_len:
			#	i_vector.append(i_vector_u)
			#else:
			#	i_vector.append(sample(i_vector_u, self.trunc_len))
			i_vector_l.append(min(len(i_vector), self.trunc_len))

			#u_vector_u = self.u_vector(t[0])
			#if len(u_vector_u) <= self.trunc_len:
			#	u_vector.append(u_vector_u)
			#else:
			#	u_vector.append(sample(u_vector_u, self.trunc_len))
			u_vector_l.append(min(len(u_vector), self.trunc_len))
			...
			uf_read_linput.append(uf_read_l_u)
			...
			if_read_linput.append(if_read_l_i)

		data_len = len(iid)
    
		#padding
		u_vector_maxlength = min(self.trunc_len, max(u_vector_l)) #500
		u_vectorinput = np.zeros([data_len, u_vector_maxlength], dtype = np.int32)
		for i, ru in enumerate(u_vector):
			u_vectorinput[i, :min(len(ru), u_vector_maxlength)] = ru[:min(len(ru), u_vector_maxlength)]

		i_vector_maxlength = min(self.trunc_len, max(i_vector_l)) #500
		i_vectorinput = np.zeros([data_len, i_vector_maxlength], dtype = np.int32)
		for i, fi in enumerate(u_vector):
			i_vectorinput[i, :min(len(fi), i_vector_maxlength)] = fi[:min(len(fi), i_vector_maxlength)]

		u_read_maxlength = max(u_read_l)
		u_friend_maxlength = min(self.trunc_len, max(u_friend_l)) #500
		uf_read_maxlength = min(self.trunc_len, max(max(uf_read_linput)))
		...
		for i, fr in enumerate(uf_read_linput):
		...
		if_read_maxlength = min(self.trunc_len,max(max(if_read_linput)))
		...
		for i, fr in enumerate(if_read_linput):
		...
		#return self.i, (iid, uid, label), u_readinput, u_friendinput, uf_readinput, (u_att1, u_att2, u_vector_l), u_read_l, u_friend_l, uf_read_linput, i_readinput, i_friendinput, if_readinput, (i_att1, i_att2, i_vector_l), i_linkinput, i_read_l, i_friend_l, if_read_linput,

		return self.i, (iid, uid, label), u_readinput, u_friendinput, uf_readinput, u_vectorinput, u_read_l, u_friend_l, uf_read_linput, u_vector_l, i_readinput, i_friendinput, if_readinput, i_vectorinput, i_linkinput, i_read_l, i_friend_l, if_read_linput,  i_vector_l
